...
./docker/run.sh bench               # the opt-in performance gate (measured: 21m16s)
                                    # since 2026-08-14 this is the main way it gets run:
                                    # bench.yaml is manual-only, so nothing runs it for you
./docker/run.sh --help
```
...
|---|---|---|---|
| `ci` | `ci.yaml` / `build-and-test` | **PASS** | `3079 tests, 0 failures, 6 skipped` across **280** surefire report files, all seven reactor projects `BUILD SUCCESS`. Module split: core **2621** (5 skipped), conformance **355** (1 skipped; its unit tests only — the corpus sweep is behind `-Pconformance` and does not run here), db **89**, geoapi **14**, and no test sources in `epsg` or `grids-us-legacy`. The 2.2.0 release commit `12a5db4`, measured the same way, is **3029** — core 2573, conformance 354, db 88, geoapi 14. **Trail of this number:** `1792` (`maven-javadoc-plugin:jar (attach-javadocs)` failed on `neoproj4j-db`, stopping the reactor before `db` and `conformance`), `2142` once that was fixed, `2320`, `2573`, `2635`, `2640`, `2662`, `2667`, `3029`, then `3079` — every rise is new tests, **0 pre-existing tests broken at any point**. The last two readings are 50 apart and the report-file count moved by 4 with them (276 at `3029`, 280 now), so most of this rise is new methods in classes that already existed rather than new test classes — the opposite shape to the `2667` → `3029` rise, which was 362 tests and 25 new report files. The 6 skips are 5 in `core` and 1 in `GieConformanceTest` (corpus profile off); two of core's are the documented build-order assumptions in `NoGeoApiInCoreTest` (geoapi not yet compiled, core not yet packaged, both true when surefire runs), and the other three are `EsriDatumTableTest`, which re-derives the ESRI datum table from a PROJ checkout at a pinned commit and skips when there is none — the container has no PROJ checkout, so it always skips there. |
| `conformance` | `conformance.yaml` / `corpus` | PASS | `7915/7922` genuine passes, `regressed 0`, 7,923 assertions evaluated. Those are three different quantities: 7,923 in-block corpus rows evaluated (a further 94 rows are out of block and excluded), 7,922 of them non-vacuous (1 vacuous `expect failure` row is excluded from both numerator and denominator), 7,915 of *those* passing, with 6 failing and 1 skipped. **The denominator moves as well as the numerator, and it can move against you.** It was `7449/7902` before `+proj=airocean` and `+proj=isea` were registered: those two took 135 assertions from failing to passing, and three of the 21 vacuous rows stopped being vacuous — they were only vacuous because the operator did not exist — so they joined the denominator, giving `7584/7905`. Then six more became real measurements and it moved 7,905 → 7,911, leaving 12. **In 2.3.0 it happened on the largest scale yet:** `+proj=gridshift` and `+proj=defmodel` landed together, and eleven of those twelve vacuous rows became rows that have to be earned, taking the denominator 7,911 → 7,922. They were earned, so the pair rose on both sides at once. A ratio quoted without its denominator is not comparable across releases |
| `golden` | `golden.yaml` / `golden` | **FAIL (expected)** | `2083 UNEXPLAINED`, **55/55** rules pinned, no `FIGURES_MOVED`/`COUNT_MISMATCH`/`DEAD_RULE`/`EXPIRED_RULE`/`PENDING_RULE_FIRED`. Full split `11944 UNCHANGED · 41486 CHANGED · 20 ADDED · 0 REMOVED · 39423 INTENDED` over 53,450 rows. **The 20 ADDED are new in 2.3.0** and are the four dictionary definitions this release adds × 5 probes; they are claimed by `DICT-2.3.0-FOUR-DEFS-ADDED`, and they are why the generated table is now longer than the frozen baseline. What is gated is that this report is *unchanged against the baseline*, not that the test passes, so a green `golden` is not what this check is waiting for |
| `determinism` | `determinism.yaml` / `bits` (one leg) | PASS | `22 tests, 0 failures` |
| `bench` | `bench.yaml` / `gate` — **which is turned off for now: `workflow_dispatch` only, no push, no pull request, no schedule. Nothing in CI runs it, so this local check and a hand-started CI run are the only cover** | PASS | `0 breaches`, **`245 gated, 0 EXCLUDED`, 245 arms**, `245 of 245 arms carry an allocation measurement`, in **21 m 16 s**. Was `172 gated, 9 EXCLUDED, 181 arms` earlier: **+64 arms** because `BulkTransformBenchmark` came out of the `staged` package and into the gate, and **−9 exclusions** because `crs-parse` rejoined Tier 1 when `io/InitFileCache` removed the per-call re-scan that made it flake |
...
* **`golden`** is an expected failure *only while its failure is `UNEXPLAINED` rows and nothing
  ...
  rather than only naming it. The 2,083 unexplained rows are a triage backlog owned by the streams
...
added. The number is a floor that only ever goes up: it is there to catch tests going missing, so
lowering it to match a smaller run defeats the whole point of having it.
...
**The transcript below is from 2026-08-01 and is quoted unedited, for the same reason §2's is: it is
a recorded measurement, not an example. Every figure in it is stale — the pair reads `7915/7922`
today, and the key it breaks, `gie/4D-API_cs2cs-style.gie#11:0`, is not even in the manifest any
more, because `helmert` was implemented in 2.2.0. Read it for the shape of the control, not for its
numbers.** The control itself is not stale: it picks whatever row it finds rather than that key.
...
|---|---|---|
| `ci` | ≥ 3,000 tests across all surefire XMLs | **3,079 tests**, 0 failures, 6 skipped *(was 3,029; 2,667; 2,662; 2,640; 2,635; 2,573; 2,320; 2,142; 1,792 before the `neoproj4j-db` javadoc fix let `db` and `conformance` reach their test phases)*. **The floor only ever goes up — raise it deliberately, never lower it.** The property it is chosen for is that it still fails when `db` drops out of the reactor. `db` is **89** tests now and 3,079 − 89 = 2,990, still under 3,000, so the property holds — **but by 10 tests, down from 59 at 2.2.0.** 3,000 is the only round hundred in the admissible interval (2,990, 3,079] — 2,900 fails to catch a missing `db` and 3,100 fails on a healthy tree — so it is forced rather than preferred. **Whoever pushes the total past 3,088 has to raise this floor to 3,100 in the same change**, because at 3,089 a db-less run reads 3,000 and passes. Ten tests is one ordinary test class, so expect that to be the next release. It still cannot also cover `geoapi`'s 14 and leave any headroom. Earlier: the floor was 2,600, which had lost the property by 2.2.0, and 2,500 before that, chosen against the 2,573 reading and outgrown the same way — 2,560 was 60 clear of it, so `db` could have vanished green. Before that it was 1,700. |
| `conformance` | `GieConformanceTest` ≥ 7,900 tests; ≥ 25 other classes; `evaluated == index keys`; 0 skips outside the sweep; `skipped == still_failing` | **7,971 sweep tests**, **7,923 assertions evaluated** against a 7,923-key index, 27 other classes, 0 outside skips, 8 == 8 *(was 104 == 104 before the two grid operators landed; the manifest shrank from 104 data rows to 8, and this equality is the guard that the sweep skipped exactly the rows the manifest excuses and no others)* |
| `golden` | exactly 1 `GoldenMasterTest`, 0 skips; generated line count == baseline + the declared surplus; ≥ 55 tests in the module; the six diff figures equal `baseline/1.4.3/golden-expect.txt` | **53,451 lines** generated vs 53,431 baseline **+ a declared surplus of 20** — those are `wc -l` of files whose first line is a header, so **53,450 data rows generated against 53,430 pinned**. The surplus is the four dictionary definitions 2.3.0 adds × 5 probes; the check stays two-sided, so 19 rows and 21 rows both fail. Set the surplus back to 0 when the baseline is refreshed. **64 tests**, 0 skips. The figure check lives in `GoldenMasterTest`, not here, because `golden.yaml` is `schedule`-only and gates nothing on a PR |
