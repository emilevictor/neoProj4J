...
|---|---|---|---|
| `ci` | `ci.yaml` / `build-and-test` | **PASS** | `3029 tests, 0 failures, 6 skipped` across **276** surefire report files, all seven reactor projects `BUILD SUCCESS`. Module split: core **2573** (5 skipped), conformance **354** (1 skipped; its unit tests only — the corpus sweep is behind `-Pconformance` and does not run here), db **88**, geoapi **14**, and no test sources in `epsg` or `grids-us-legacy`. Master at `2fc5989`, measured the same way, is **2640**. **Trail of this number:** `1792` (`maven-javadoc-plugin:jar (attach-javadocs)` failed on `neoproj4j-db`, stopping the reactor before `db` and `conformance`), `2142` once that was fixed, `2320`, `2573`, `2635`, `2640`, `2662`, `2667`, then `3029` — every rise is new tests, **0 pre-existing tests broken at any point**. The last two readings are 362 apart and the report-file count moved with them (251 at both `2662` and `2667`, 276 now), so this rise is new test classes as well as new methods in classes that already existed. The 6 skips are 5 in `core` and 1 in `GieConformanceTest` (corpus profile off); two of core's are the documented build-order assumptions in `NoGeoApiInCoreTest` (geoapi not yet compiled, core not yet packaged, both true when surefire runs), and the other three are `EsriDatumTableTest`, which re-derives the ESRI datum table from a PROJ checkout at a pinned commit and skips when there is none — the container has no PROJ checkout, so it always skips there. |
| `conformance` | `conformance.yaml` / `corpus` | PASS | `7819/7911` genuine passes, `regressed 0`, 7,923 assertions evaluated. Those are three different quantities: 7,923 in-block corpus rows evaluated (a further 94 rows are out of block and excluded), 7,911 of them non-vacuous (12 vacuous `expect failure` rows are excluded from both numerator and denominator), 7,819 of *those* passing (98.84%), with 90 failing and 2 skipped. **The denominator moves as well as the numerator, and it can move against you.** It was `7449/7902` before `+proj=airocean` and `+proj=isea` were registered: those two took 135 assertions from failing to passing, and three of the 21 vacuous rows stopped being vacuous — they were only vacuous because the operator did not exist — so they joined the denominator, giving `7584/7905`. **The same thing has happened again:** six more of those 18 remaining vacuous rows are real measurements now, so the denominator moved 7,905 → 7,911 for exactly that reason, leaving 12. A ratio quoted without its denominator is not comparable across releases |
| `golden` | `golden.yaml` / `golden` | **FAIL (expected)** | `2083 UNEXPLAINED`, **54/54** rules pinned, no `FIGURES_MOVED`/`COUNT_MISMATCH`/`DEAD_RULE`/`EXPIRED_RULE`/`PENDING_RULE_FIRED`. Full split `11944 UNCHANGED · 41486 CHANGED · 0 ADDED · 0 REMOVED · 39403 INTENDED` over 53,430 rows. What is gated is that this report is *unchanged against the baseline*, not that the test passes, so a green `golden` is not what this check is waiting for |
...
|---|---|---|
| `ci` | ≥ 3,000 tests across all surefire XMLs | **3,029 tests**, 0 failures, 6 skipped *(was 2,667; 2,662; 2,640; 2,635; 2,573; 2,320; 2,142; 1,792 before the `neoproj4j-db` javadoc fix let `db` and `conformance` reach their test phases)*. **The floor only ever goes up — raise it deliberately, never lower it.** The property it is chosen for is that it still fails when `db` drops out of the reactor. **The floor was raised 2,600 → 3,000 in this change, because at 2,600 that property had been lost:** `db` is **88** tests now and 3,029 − 88 = 2,941, comfortably over 2,600. 3,000 is the only round hundred in the admissible interval (2,941, 3,029] — 2,900 fails to catch a missing `db` and 3,100 fails on a healthy tree — so it is forced rather than preferred, and it holds by 59 tests. **Whoever pushes the total past 3,087 has to raise this floor in the same change**, because at 3,088 a db-less run reads 3,000 and passes. It still cannot also cover `geoapi`'s 14 and leave any headroom. Earlier readings: 2,500, chosen against the 2,573 reading, which the suite outgrew — 2,560 was 60 clear of it, so `db` could have vanished green. Before that it was 1,700. |
| `conformance` | `GieConformanceTest` ≥ 7,900 tests; ≥ 25 other classes; `evaluated == index keys`; 0 skips outside the sweep; `skipped == still_failing` | **7,971 sweep tests**, **7,923 assertions evaluated** against a 7,923-key index, 27 other classes, 0 outside skips, 104 == 104 |
